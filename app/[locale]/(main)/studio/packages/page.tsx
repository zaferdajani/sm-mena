    ...
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {rows.map((p) => (
        <div key={p.id} className="space-y-1">
          <PackageForm services={services} platforms={platforms} initial={p} />
      ...
      {rows.length < MAX_PACKAGES ? <PackageForm key={`new-${rows.length}`} services={services} platforms={platforms} /> : <p className="text-sm text-muted-foreground">{t("studio.limit")}</p>}
    </div>
  );
}
