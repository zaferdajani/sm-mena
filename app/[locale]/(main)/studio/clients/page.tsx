  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      {clients.map((c) => (
        <ClientItem
          key={c.id}
          client={{ id: c.id, name: c.name, industry: c.industry, country: c.country, description: c.description, links: c.links }}
      ...
      {clients.length < MAX_CLIENTS ? (
        <section className="space-y-2">
          ...
          <ClientForm key={`new-${clients.length}`} industries={industries} countries={countries} defaultCountry={agency.country} />
        </section>
      ) : (
