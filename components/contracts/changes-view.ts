...
export const changesFor = (v: ContractView, locale: string) =>
  v.changes.map((c) => ({ id: c.id, title: c.title, reason: c.reason, amount: formatFils(c.amountFils, locale), dueDate: c.dueDate, checks: c.checks, status: c.status, decidedBy: c.decidedBy }));
