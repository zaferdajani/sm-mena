...
## 6. What the product needs (owner decision: build order)

1. **Behind-the-Page card**: an image per agency (and per account) generated by the site: logo, member number, the accounts they handle with logos, QR/link to the page. One tap → saved to phone / shared to story. *(Small: an OG-style image route.)*
2. **Client confirmation**: the agency sends a link; the client's page owner confirms (email or a code posted in the page bio). Confirmed accounts get the badge; only confirmed accounts appear in the "who runs this page" search. *(Medium.)*
3. **Who runs this page?** search: `/who-runs` accepting @handle or a link; matches confirmed account links. *(Small once 2 exists.)*
4. **Member number** on the agency page and card; **Sawwiq 50** page computed from confirmed accounts + verified reviews. *(Small.)*
