...
| **Founding 100 with a concrete offer**, and separate *membership number* from *founding-cohort eligibility* from *benefits received*. | Agreed. Member numbers (docs/28) are a join sequence and say so; the offer needs cohort dates, a cap on hands-on benefits, activation and expiry. | **Done:** `lib/founding.ts` (cohort = first 100 seats joined before `FOUNDING_CLOSES_AT`; benefits from `FOUNDING_ACTIVATED_AT` for 90 days), dated badge on the page header, Founding panel in the studio naming each benefit and its boundary; 01 §10 rewritten; the teaser counter says "registered provider profiles". |
...
| **Research first**: 20 buyer and 15 provider interviews in Jordan; test three provider messages. | Agreed. | **Done:** `marketing/07-interviews.md` (scripts, who, how to read the results). Owner: name who runs them. |
...
| **SEO**: price guide from comparable packages, not agency starting prices; editorial indexing over scaled city×service pages; country-correct OG locale; don't sell FAQ markup or llms.txt as growth. | Agreed on all four. | **Done:** price guide from comparable packages (monthly and one-off separately, service-specific, ≥3 samples; starting prices only as a labelled fallback); `og:locale` by country. Indexing thresholds unchanged for now (few pages exist); docs/18 wording updated. |
| **Hosting**: Vercel Hobby is for personal, non-commercial use; a free-to-users marketplace is still commercial. | Correct. docs/27 deferred Pro until charging; that reading was too generous. | README and docs/27 now say Pro. **Owner: upgrade the Vercel project before public launch.** |
